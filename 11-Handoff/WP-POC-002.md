...
- Minimal human action: privately provision the four `GREEN_API_*` values, expose/configure the HTTPS webhook, connect the dedicated number, add it to an allowed ordinary group with another participant, and publish the source message. The automated endpoints can then execute the send; a participant must record whether `PEGO` visibly quotes that exact message.
