...
- Completed WPs: `WP-POC-001`, `WP-POC-002`, `WP-POC-003`, `WP-POC-004`.
- Next gate: `WP-POC-006` claim engine, which consumes the `ELIGIBLE` opportunities the rule engine now produces and is the first stage allowed to send a `PEGO` on its own.
