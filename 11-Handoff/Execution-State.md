...
- Completed WPs: `WP-POC-001`, `WP-POC-002`, `WP-POC-003`, `WP-POC-004`, `WP-POC-005`.
- Next gate: `WP-POC-007` reliability and observability — restart recovery, provider health gating the automatic path, and latency percentiles.
- Nothing sends a `PEGO` without an explicit `EP-023` call: no automatic trigger is wired, and `AUTO` mode additionally requires a stored evaluation with `autoClaimAllowed`. Rule set v1 in production sets `autoClaimEnabled=false` and the group flag is off.
